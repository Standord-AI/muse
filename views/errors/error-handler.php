...
<head>
    <title><?= $errtype . ' - ' . config('APP_NAME') ?></title>
...
<body>
    ...
    <section class="bg-white dark:bg-gray-900 flex h-screen">
        <div class="py-8 px-4 max-w-screen-xl w-full lg:py-16 lg:px-6 m-auto">
            <div class="mx-auto max-w-screen-sm w-full">
                ...
                <p class="mb-4 text-lg font-light text-gray-500 dark:text-gray-400">Fix the below error in order to run the <?= config('APP_NAME') ?>.</p>

                <?php if ($errno == E_ERROR) : ?>
    ...
    <script>
        console.error(`\t<?= $errtype ?> : <?= str_replace("\\", "\\\\", $errstr) ?>\n\tline <?= $errline ?> : of the file <?= str_replace("\\", "\\\\", $errfile) ?>\n\tPHP <?= PHP_VERSION ?> (<?= PHP_OS ?>)\n`);
    </script>
